...
def super_simple():
    return jsonify(message='hello from the planetary new one')
...
def parameters():
    ...
    if age < 18:
        ...
        return jsonify(message=f'all good {name} you are old enough')
...
#here we can tell which WORD to acceps
...
#this one is for registering new users
...
#this one is for login with JWT
...
def add_planet():
    ...
    if test:
        return jsonify(message='There is already planet with that name'), 409
    else:
...
#we can serialize single object or collection of objects
